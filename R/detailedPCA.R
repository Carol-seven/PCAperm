...
#' the eigenvectors).}
#' \item{indexload}{The index of loadings for the variables.}
#' \item{correlation}{The correlations of the variables with each principal component.}
...
detailedPCA <- function(X, center = TRUE, scale = TRUE) {
  ...
  ## the index loadings of the variables
  ...
  return(list(X = X, sdev = sdev, eigval = eigval, pctvar = pctvar,
              rotation = rotation, indexload = indexload, correlation = correlation,
              Psi = Psi, Phi = Phi, center = centering.used, scale = scaling.used))
}
